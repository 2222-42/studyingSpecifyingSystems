...
RespondToRd(proc, reg) ==
    ...
    /\ \E val \in Val:
            ...
            /\ opQ' = LET idx == CHOOSE i \in DOMAIN opQ[proc] : opQ[proc][i].reg = reg
                      IN opQ' = [opQ EXCEPT ![proc][idx].reg.val = val ,
                                            ![proc][idx].reg = Done]
    /\ UNCHANGED mem
...
RemoveOp(proc) ==
    ...
    /\ mem' = IF Head(opQ[proc]).req.op = "Rd"
                THEN mem
                ELSE [mem EXCEPT ![Head(opQ[proc]).req.adr] = Head(opQ[proc]).req.val]
    /\ opQ' = [opQ EXCEPT ![proc] = Tail(@)]
    /\ UNCHANGED regFile

Internal(proc) ==
    /\ RemoveOp(proc)
    /\ (Head(opQ[proc]).req.op = "Rd") => (mem[Head(opQ[proc]).req.adr] = Head(opQ).req.val)

Next ==
    \E proc \in Proc :  \/ \E reg \in Reg : \/ \E req \in Request: IssueRequest(proc, req, reg)
                                           \/ RespondToRd(proc, reg)
                                           \/ RespondToWr(proc, reg)
                        \/ Internal(proc)
...
Spec == /\ Init
        ...
        /\ \A proc \in Proc, reg \in Reg : 
            WF_<<regFile, opQ, mem>>(RespondToWr(proc, reg) \/ RespondToRd(proc, reg))
        /\ \A proc \in Proc :
            WF_<<regFile, opQ, mem>>(RemoveOp(proc))
...
\* Last modified Sun May 03 14:42:42 JST 2020 by daioh
\* Created Sat May 02 14:16:35 JST 2020 by daioh
